  ...
  return (
    <div
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-accent ring-1 ring-primary/20 shadow-sm' : 'hover:bg-secondary'}`}
    ...
    >
      ...
      <div className="relative flex-1 py-0.5 px-1">
        ...
        {showCommands && (
          <div
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-popover text-popover-foreground rounded-lg shadow-lg border border-border py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-secondary
