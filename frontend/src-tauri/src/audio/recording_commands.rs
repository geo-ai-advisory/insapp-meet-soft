...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    // ДОСТУП К МИКРОФОНУ. Система (и macOS, и Windows) молча не отдаёт звук, если
    // доступ не выдан - без явной проверки пользователь видел только «Не удалось
    // начать запись» и не понимал, что делать. Доступ слетает, например, после
    // переустановки приложения или обновления системы.
    // Здесь мы ПРОВОЦИРУЕМ системный запрос доступа и, если его нет, говорим прямо.
    match tauri::async_runtime::spawn_blocking(crate::audio::trigger_audio_permission).await {
        Ok(Ok(true)) => info!("✅ Доступ к микрофону есть"),
        Ok(Ok(false)) | Ok(Err(_)) | Err(_) => {
            let user_msg = if cfg!(target_os = "macos") {
                "Нет доступа к микрофону. Разреши его приложению: Системные настройки → Конфиденциальность и безопасность → Микрофон."
            } else {
                "Нет доступа к микрофону. Разреши его приложению в параметрах конфиденциальности Windows (Микрофон → доступ для классических приложений)."
            };
            error!("❌ Доступ к микрофону не получен - запись не начата");
